import { cart, setCart } from "@/features/restaurant/store/cartStore";
...
export function addToCart(mealId: string) {
  setCart(items => {
    ...
    if (existing) {
      return items.map(i => (i.mealId === mealId ? { ...i, quantity: i.quantity + 1 } : i));
    }
...
export const cartItems = createMemo(() => {
  const mealsById = Object.entries(mealModel.meal()).flatMap(([menuName, categories]) =>
  ...
  return cart
    .map(cartItem => {
      const matchingMeal = mealsById.find(meal => meal.mealId === cartItem.mealId);
